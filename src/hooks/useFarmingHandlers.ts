    ...
    const eternalCollectRewardHandler = useCallback(async (token, {
        ...
        try {
            ...
            if (eternalRewardToken.id.toLowerCase() !== eternalBonusRewardToken.id.toLowerCase()) {
                result = await farmingCenterContract.multicall([collectRewards, claimReward1, claimReward2])
            } else {
                result = await farmingCenterContract.multicall([collectRewards, claimReward1])
            }
