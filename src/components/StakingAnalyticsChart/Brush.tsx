...
export default function Brush({ data, data2, colors, focusHeight, width, margin, updateChartData, X }: BrushProps) {
    ...
    useEffect(() => {
        ...
        const focus = focusEl.attr("viewBox", `0, 0, ${width}, ${focusHeight}`).style("display", "block");

        const focusLine = (x: any, y: any) =>
            line()
                .defined((d) => {
                    // @ts-ignore
        ...
        // @ts-ignore
        const focusXAxis = (g, x, height) => g.attr("transform", `translate(0,${height - margin.bottom})`);
        // @ts-ignore
        const focusYAxis = (g, y, title) =>
            g
                .attr("transform", `translate(${margin.left},0)`)
                .call(axisLeft(y))
                .call((g: any) => g.select(".domain").remove())
                .call((g: any) =>
                    g.selectAll(".title").data([title]).join("text").attr("class", "title").attr("x", -margin.left).attr("y", 10).attr("fill", "currentColor").attr("text-anchor", "start").text(title)
                );

        focus.append("g").call(focusXAxis, focusX, focusHeight);
        ...
        const gb = focus.append("g").call(brush).call(brush.touchable, [1, 7]).call(brush.move, defaultSelection);

        gb.select(".selection").attr("fill", "#000").attr("stroke", "none").attr("rx", "5");
    ...
    return <svg ref={focusRef} width={width} height={focusHeight} />;
}
