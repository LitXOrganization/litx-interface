...
export function FarmingEventCard({
            ...
            <div className={"farming-event-card p-1 br-12"} data-refreshing={refreshing}>
                <div className={"f mb-1"}>
                    <div className={"f mr-1"}>
                        <CurrencyLogo
                            currency={new Token(AlgebraConfig.CHAIN_PARAMS[chainId || 56].chainId, "0x580a84c73811e1839f75d86d75d88cca0c241ff4", 18, "QI") as WrappedCurrency}
                            size={"30px"}
                        />
                        <CurrencyLogo
                            currency={new Token(AlgebraConfig.CHAIN_PARAMS[chainId || 56].chainId, "0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270", 18, "MATIC") as WrappedCurrency}
                            size={"30px"}
                        />
                    </div>
                    <div>
                    ...
                    <ul className="farming-event-card__reward-list">
                        {[{ token: { id: "0xa3fa99a148fa48d14ed51d610c367c61876997f1", symbol: "MAI" }, amount: "2500" }].map((reward: any, i) => (
                            <li key={i} className="farming-event-card__reward-list-item f">
                                <CurrencyLogo currency={new Token(AlgebraConfig.CHAIN_PARAMS[chainId || 56].chainId, reward.token.id, 18, reward.token.symbol) as WrappedCurrency} size={"30px"} />
    ...
    return (
        <div className={"farming-event-card p-1 br-12"} data-refreshing={refreshing}>
            {refreshing && (
                <LoadingShim>
                    <Loader size={"18px"} stroke={"var(--loader)"} style={{ margin: "auto" }} />
                </LoadingShim>
            )}
            {locked && (
                <LoadingShim>
                    <div style={{ padding: "1rem", background: "#fff", borderRadius: "8px", color: "black" }}>
                        <div className="mb-1 f f-ac f-jc">
                            <span className="mr-05">
                                <Lock size={"16px"} stroke={"var(--loader)"} />
                            </span>
                            <span>
            ...
            <div className={"f mb-1"}>
                <div className={"f mr-1"}>
                    <CurrencyLogo currency={new Token(AlgebraConfig.CHAIN_PARAMS[chainId || 56].chainId, pool.token0.id, 18, pool.token0.symbol) as WrappedCurrency} size={"30px"} />
                    <CurrencyLogo currency={new Token(AlgebraConfig.CHAIN_PARAMS[chainId || 56].chainId, pool.token1.id, 18, pool.token1.symbol) as WrappedCurrency} size={"30px"} />
                </div>
                <div>
                    <h3 className={"fs-075 b"}>
                        <Trans>POOL</Trans>
