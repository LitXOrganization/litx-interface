...
export function FarmModal({
    ...
    const balance = useCurrencyBalance(
        account ?? undefined,
        multiplierToken ? new Token(AlgebraConfig.CHAIN_PARAMS[chainId || 56].chainId, multiplierToken.id, +multiplierToken.decimals, multiplierToken.symbol, multiplierToken.name) : undefined
    );
    ...
    const _amountForApprove = useMemo(() => {
        ...
        return CurrencyAmount.fromRawAmount(
            new Token(AlgebraConfig.CHAIN_PARAMS[chainId || 56].chainId, multiplierToken.id, +multiplierToken.decimals, multiplierToken.symbol, multiplierToken.name),
            selectedTier
        );
    }, [selectedTier, multiplierToken]);

    const [approval, approveCallback] = useApproveCallback(_amountForApprove, AlgebraConfig.V3_CONTRACTS[chainId || 56].FARMING_CENTER_ADDRESS);
    ...
    return (
        <>
            {submitState === 3 ? (
                ...
                <div className={`w-100 ${!isTierFarming && "h-400 pos-r"}`}>
                    ...
                    <div style={{ height: "inherit", marginLeft: "-1rem", position: "relative", marginRight: "-1rem" }} className="mb-1 pl-1 pr-1">
                        {chunkedPositions && chunkedPositions.length === 0 ? (
                            <div className={`f c f-ac f-jc full-h`}>
                                <Frown size={30} stroke={"var(--icon)"} />
                            ...
                            chunkedPositions.map((row, i, arr) => (
                                <div style={{ opacity: !isEnoughTokenForLock && selectedTier ? "0.5" : "1" }} className="f mb-1 pl-1 pb-1 pr-1 mxs_pb-0 farm-modal__nft-position-row" key={i}>
                                    {row.map((el, j) => (
                                        <div
                                            className={"farm-modal__nft-position p-1 br-8"}
                                            key={j}
                                        ...
                                        >
                                            ...
                                            <div className="ml-1">
                                                ...
                                                <div className={"farm-modal__nft-position__description"}>
                                                    <Link className={"fs-085 hover-cp"} to={`/pool/${+el.id}`}>
